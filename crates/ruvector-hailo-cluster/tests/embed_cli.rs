...
fn embed_cli_text_flag_emits_json_line() {
    ...
    assert!(line.contains("\"text\":\"hello\""), "missing text field: {:?}", line);
    assert!(line.contains("\"dim\":4"), "missing dim field: {:?}", line);
    assert!(line.contains("\"vec_head\":["),
        "missing vec_head field (default --output mode): {:?}", line);
}
...
fn embed_cli_output_full_emits_full_vector() {
    ...
    assert!(line.contains("\"vector\":["),
        "expected full vector field, got {:?}", line);
    ...
    assert_eq!(comma_count, 7,
        "expected 7 commas for dim=8, got {} in {:?}", comma_count, vec_segment);
}
...
fn embed_cli_version_flag_prints_pkg_name_and_version() {
    ...
    for arg in &["--version", "-V"] {
        ...
        assert!(out.status.success(), "embed {} exited {:?}", arg, out.status);
        let stdout = String::from_utf8_lossy(&out.stdout);
        let line = stdout.trim();
        assert!(line.starts_with("ruvector-hailo-cluster"),
            "expected pkg-name prefix, got: {:?}", line);
        let parts: Vec<&str> = line.split_whitespace().collect();
        assert_eq!(parts.len(), 2, "expected `<name> <version>`, got: {:?}", line);
        // Version should be a semver-ish string (digits + dots).
        assert!(parts[1].chars().all(|c| c.is_ascii_digit() || c == '.' || c == '-' || c.is_ascii_alphabetic()),
            "version field looks malformed: {:?}", parts[1]);
    }
}
...
fn embed_cli_validate_fleet_with_wrong_fingerprint_exits_nonzero() {
    ...
    let out = Command::new(EMBED)
        .args([
            "--workers", &format!("127.0.0.1:{}", port),
            "--dim", "4",
            "--fingerprint", "fp:not-the-fakeworker-fp",
            "--validate-fleet",
            "--validate-only",
            "--quiet",
        ])
